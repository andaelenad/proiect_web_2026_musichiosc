...
window.addEventListener("load", function () {
  ...
  var btns = [];
  var b1 = document.getElementById("btn-schimba-tema");
  var b2 = document.getElementById("schimba-tema");
  if (b1) btns.push(b1);
  if (b2) btns.push(b2);
  var themes = ["light", "dark", "ocean"]; // cycle order

  var tema = localStorage.getItem("tema"); // fallback to cookie 'tema' if localStorage is empty (server may have set it)
  ...
  if (radios && radios.length) {
    ...
    var curTheme = document.documentElement.getAttribute('data-bs-theme') || 'light';
    var active = document.querySelector('input[name="theme"][value="' + curTheme + '"]');
  ...
  function schimbaIcon(tema) {
    ...
    if (!icon && b2) icon = b2.querySelector("i"); // fallback: dedicated icon element used in some pages
